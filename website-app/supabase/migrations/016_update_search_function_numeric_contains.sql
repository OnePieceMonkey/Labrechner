-- ################################################
-- Migration 016: Allow numeric substring search for position_code
-- ################################################
...
DECLARE
  normalized_query TEXT := regexp_replace(COALESCE(search_query, ''), '\s+', '', 'g');
  is_numeric_query BOOLEAN := normalized_query ~ '^\d+$';
BEGIN
  RETURN QUERY
  SELECT
    bp.id,
    ...
    CASE
      ...
      ELSE
        GREATEST(
          ts_rank(
            to_tsvector('german', bp.name || ' ' || COALESCE(bp.description, '')),
            plainto_tsquery('german', search_query)
          ),
          similarity(bp.name, search_query),
          CASE
            WHEN is_numeric_query AND bp.position_code ILIKE '%' || normalized_query || '%' THEN 1.0
            WHEN NOT is_numeric_query AND bp.position_code ILIKE search_query || '%' THEN 1.0
            ELSE 0.0
          END
        )::REAL
    END AS rank
  ...
  WHERE
    (
      ...
      OR (is_numeric_query AND bp.position_code ILIKE '%' || normalized_query || '%')
      OR (NOT is_numeric_query AND bp.position_code ILIKE search_query || '%')
      OR bp.name ILIKE '%' || search_query || '%'
    )
    AND (group_filter IS NULL OR bp.group_id = group_filter)
  ORDER BY
    CASE WHEN search_query = '' OR search_query IS NULL THEN 0 ELSE 1 END,
    rank DESC,
