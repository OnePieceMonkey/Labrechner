...
import { Sparkles, Mic, FileText, Bot, User } from "lucide-react";
...
const INVOICE_ITEMS: InvoiceItem[] = [
  { id: 1, code: "0010", name: "Modell", price: 14.5, type: "manual" },
  { id: 2, code: "0120", name: "Mittelwertartikulator", price: 22.1, type: "ai" },
  { id: 3, code: "0212", name: "Guss der Basis", price: 45.8, type: "manual" },
  { id: 4, code: "3800", name: "Verblendung Keramik", price: 120.0, type: "voice" },
  { id: 5, code: "9330", name: "Versandkosten", price: 6.9, type: "ai" },
];
...
export function InvoiceAnimation() {
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto">
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl shadow-brand-900/10 border border-gray-200 dark:border-slate-800 overflow-hidden relative transition-colors duration-300">
        ...
        <div className="p-6 min-h-[380px] bg-white dark:bg-slate-900 flex flex-col gap-3 transition-colors">
          ...
          {visibleItems.map((id, index) => {
            ...
            let containerClass =
              "flex items-center p-3 rounded-lg border text-sm transition-all ";
            let icon = null;

            if (item.type === "ai") {
              containerClass +=
                "bg-purple-50/50 dark:bg-purple-500/10 border-purple-100 dark:border-purple-500/20 text-slate-800 dark:text-purple-100 shadow-sm shadow-purple-100/50 dark:shadow-none";
              icon = (
                <Sparkles className="w-3.5 h-3.5 text-purple-600 dark:text-purple-400 animate-pulse" />
              );
            } else if (item.type === "voice") {
              containerClass +=
                "bg-blue-50/50 dark:bg-blue-500/10 border-blue-100 dark:border-blue-500/20 text-slate-800 dark:text-blue-100 shadow-sm shadow-blue-100/50 dark:shadow-none";
              icon = <Mic className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />;
            } else {
              containerClass +=
                "bg-white dark:bg-slate-800 border-gray-100 dark:border-slate-700 text-slate-700 dark:text-slate-300";
            ...
            return (
              ...
              >
                <div className="w-16 font-mono text-xs text-slate-500 dark:text-slate-500">
                  {item.code}
                </div>
                <div className="flex-1 font-medium flex items-center gap-2">
                  {item.name}
                  {item.type === "ai" && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 font-semibold tracking-wide uppercase">
                      AI
                    </span>
                  )}
                  {item.type === "voice" && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-300 font-semibold tracking-wide uppercase">
                      Voice
                    </span>
                  )}
                </div>
                <div className="w-20 text-right font-mono text-slate-600 dark:text-slate-400">
                  {item.price.toFixed(2)} €
                </div>
                <div className="w-8 flex justify-end">{icon}</div>
              </div>
            );
          })}
      ...
      {/* Floating Labels */}
      <div
        className="absolute -right-4 top-20 bg-white/80 dark:bg-slate-800/90 backdrop-blur px-3 py-1.5 rounded-lg shadow-sm border border-purple-100 dark:border-purple-500/30 text-xs text-purple-700 dark:text-purple-300 font-medium rotate-6 hidden md:block animate-fade-in transition-colors"
        style={{ animationDelay: "1.2s" }}
      >
        ✨ KI-Vorschlag
      </div>
      <div
        className="absolute -left-4 top-40 bg-white/80 dark:bg-slate-800/90 backdrop-blur px-3 py-1.5 rounded-lg shadow-sm border border-blue-100 dark:border-blue-500/30 text-xs text-blue-700 dark:text-blue-300 font-medium -rotate-3 hidden md:block animate-fade-in transition-colors"
