...
export const Pricing: React.FC<PricingProps> = ({ onCheckout }) => {
  ...
  const plans = [
    ...
    },
    {
      ...
      priceMonthly: 29,
      priceYearly: 24, // 20% off approx
      features: ['Alles in Starter', 'Unbegrenzte Favoriten', 'KI-Assistent (Basis)', 'Rechnungsvorlagen'],
      cta: 'Pro buchen',
      highlight: true,
    },
    {
      ...
      priceMonthly: 99,
      priceYearly: 79, // 20% off approx
      features: ['Alles in Pro', 'KI-Assistent (Unlimited)', 'Team-Verwaltung', 'Priorisierter Support', 'API-Zugriff'],
      cta: 'Business buchen',
      highlight: false,
    },
  ];
  ...
  return (
    <section id="pricing" className="py-24 bg-white dark:bg-slate-950 border-t border-gray-100 dark:border-slate-900 transition-colors">
      <div className="container mx-auto px-6 max-w-7xl">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-5xl font-bold text-slate-900 dark:text-white mb-6 tracking-tight">
            Transparente Preise. <br />
            <span className="text-brand-600 dark:text-brand-400">Keine versteckten Kosten.</span>
          </h2>
            ...
            >
              ...
              <div className="mb-8">
                ...
                {interval === 'yearly' && plan.priceMonthly > 0 && (
                   <div className={`text-xs mt-1 ${plan.highlight ? 'text-green-400' : 'text-green-600'}`}>
                      {((plan.priceMonthly * 12) - (plan.priceYearly * 12)).toFixed(0)}€ Ersparnis im Jahr
                   </div>
                )}
              </div>
              ...
              <ul className="space-y-4 mb-10 flex-1">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3 text-sm">
                    <div className={`mt-0.5 p-0.5 rounded-full ${plan.highlight ? 'bg-indigo-500/20 text-indigo-300' : 'bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400'}`}>
              ...
              <Button
                ...
                className={`w-full justify-center ${plan.highlight ? 'bg-white !text-slate-900 hover:bg-slate-100 border-none' : ''}`}
              >
                {plan.cta}
              </Button>
              
              {plan.id !== 'free' && (
                 <p className={`text-[10px] text-center mt-3 ${plan.highlight ? 'text-slate-400' : 'text-slate-400'}`}>
